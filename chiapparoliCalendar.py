...
FYitalyHolidaysList = [everyDaysList[k] for k in range(
    0, len(everyDaysList)) if everyDaysList[k] in it_holidays]

FYshutdownPeriods = []
chiapparoliCalendarList = [date.strftime('%Y%m%d') for date in list(
    sorted(set(everyDaysList) - set(FYitalyHolidaysList))
)]
